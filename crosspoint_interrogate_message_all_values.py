...
for matrix in range(1, 17):
    for level in range(1, 17):
        for source in range(1, 101):
            for dest in range(1, 101):
                ...
                chk_data = command + byte_1 + bytes([byte_2, byte_3, btc])
                chk = compute_checksum(chk_data)
                message = som + command + byte_1 + bytes([byte_2, byte_3, btc, chk]) + eom
